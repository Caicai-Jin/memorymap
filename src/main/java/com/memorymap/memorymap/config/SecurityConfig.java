...
public class SecurityConfig {
    ...
    // Comma-separated list, e.g. "http://localhost:5173,https://memorymap.onrender.com" —
    // set via the CORS_ALLOWED_ORIGINS env var in production; defaults to local dev only.
    ...
    // Browser fetch() calls are cross-origin here (frontend on :5173, backend on :8080),
    // so the browser blocks responses unless the server explicitly allows the origin via
    // CORS headers. This is separate from Spring Security's authorizeHttpRequests rules above.
    @Bean
